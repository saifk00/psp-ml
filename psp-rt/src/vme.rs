...
#[cfg(target_os = "psp")]
impl Job {
    /// The v1.1 capability marker `VmeInit` leaves in `image_addr` ("VME1").
    const IMAGE_CAPS: u32 = 0x564D_4531;
    ...
    pub fn has_image_mode(&self) -> bool {
        // The marker survives until the first set_image; after that,
        // image_mode == 1 is itself proof of a v1.1 plugin (only v1.1 code
        // paths ever set it), covering sessions after the first one this
        // boot. Clean sessions call restore_image_caps on exit to put the
        // marker back.
        let caps = unsafe { core::ptr::read_volatile(&raw const (*self.0).image_addr) };
        let mode = unsafe { core::ptr::read_volatile(&raw const (*self.0).image_mode) };
        caps == Self::IMAGE_CAPS || mode == 1
    }

    /// Put the capability marker back (and leave image mode off) so the
    /// *next* program this boot can detect the plugin. Call before exiting
    /// a program that used [`Job::set_image`].
    pub fn restore_image_caps(&self) {
        unsafe {
            core::ptr::write_volatile(&raw mut (*self.0).image_mode, 0);
            core::ptr::write_volatile(&raw mut (*self.0).image_addr, Self::IMAGE_CAPS);
        }
    }
