...
Everything lands under ``--out`` (default ``apps/pspbird/images/``,
...
every region the app ships is one command::
    ...
    uv run apps/pspbird/fetch_images.py apps/pspbird/device/target/mipsel-sony-psp/release/blobs/*.bin
    uv run apps/pspbird/fetch_images.py a.labels.txt b.labels.txt --report
...
def write_outputs(out: str, manifest: dict) -> None:
  ...
  with open(os.path.join(out, "manifest.toml"), "w", encoding="utf-8") as f:
    f.write("# Generated by apps/pspbird/fetch_images.py -- do not edit.\n")
  ...
  with open(os.path.join(out, "IMAGE_CREDITS.txt"), "w", encoding="utf-8") as f:
    f.write("Species photos, fetched from iNaturalist by apps/pspbird/fetch_images.py.\n"
...
def main() -> int:
  ...
  p.add_argument("-o", "--out", default=os.path.join(os.path.dirname(__file__), "images"),
                 help="output directory (default: apps/pspbird/images)")
