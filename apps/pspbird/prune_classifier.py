...
Run from the repo root::
    ...
    # explicit list
    uv run apps/pspbird/prune_classifier.py MODEL.tflite labels.txt filtered.txt -o pruned.tflite

    # top 500 species for eastern North America
    uv run apps/pspbird/prune_classifier.py MODEL.tflite labels.txt --top-n 500

    # what threshold should I use?
    uv run apps/pspbird/prune_classifier.py MODEL.tflite labels.txt --report

`apps/pspbird/device/build.rs` invokes this automatically when TOPK is set.
...
# design/2026-08-24_pspbird-app.md and apps/pspbird/src/psp_bird.rs).
