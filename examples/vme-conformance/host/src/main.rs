...
//! to differ.
//!
//! Expect real-hardware mismatches at first: the assembler's skews come
//! from the RTL's pipeline model (`vme_assembler::timing`), and the real
//! array's pipeline is deeper (the proven MAC example needed a write skew
//! of 9 where the RTL needs 2). Surfacing exactly that gap, so timing.rs /
//! the RTL can be tuned against silicon, is this harness's purpose.
...
fn main() {
    ...
    if session.stdout_tail.contains("#vme fatal") {
        ...
        eprintln!("  ms0:/seplugins, and power-cycle the PSP.");
        std::process::exit(1);
    }
