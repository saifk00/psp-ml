...
fn app_main() {
    ...
    // Exit on our own after a minute with no jobs: if the host died without
    // sending quit.go we would otherwise poll a dead host0: until module!'s
    // watchdog kills this thread mid-hostfs-call -- which is exactly the
    // psplink wedge that needs a physical power-cycle (see CLAUDE.md).
    const IDLE_POLLS_MAX: u32 = 3000; // x 20 ms = 60 s
    ...
    loop {
        ...
        if !try_remove(b"host0:/job.go\0") {
            ...
            if idle_polls > IDLE_POLLS_MAX {
                dprintln!("#vme idle timeout -- exiting");
                break;
            }
        ...
        for off in WIRE_ORDER {
            ...
            for row in 0..(BUF_WORDS / 16) {
                ...
                dprintln!("");
                // Breathe every few rows: an unthrottled 147 KB burst can
                // overrun psplink's stdout forwarding and drop bytes.
                if row % 8 == 7 {
                    unsafe { sceKernelDelayThread(500) };
                }
            }
        }
    ...
    job.restore_image_caps();   // let the next program this boot detect v1.1
