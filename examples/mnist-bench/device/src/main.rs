...
struct BenchResult {
    ...
    op_profile: [psp_rt::profiler::OpProfileStats; generated::NUM_OPS],
}
...
fn run_benchmark(get_tick: fn() -> u64, tick_res: u64) -> BenchResult {
    ...
    BenchResult {
        ...
        op_ticks,
        op_profile: [psp_rt::profiler::OpProfileStats::zero(); generated::NUM_OPS],
    }
}
...
fn run_profiled_benchmark(get_tick: fn() -> u64, tick_res: u64) -> BenchResult {
    ...
    let mut predictions = [0u8; NUM_IMAGES];
    let mut op_ticks = [0u64; generated::NUM_OPS];
    let mut op_profile = [psp_rt::profiler::OpProfileStats::zero(); generated::NUM_OPS];
...
fn app_main() {
    ...
    #[cfg(feature = "profile")]
    psp_rt::dprintln!("Running PROFILED inference on {} images...", NUM_IMAGES);
    #[cfg(not(feature = "profile"))]
    psp_rt::dprintln!("Running inference on {} images...", NUM_IMAGES);
    ...
    psp_rt::dprintln!("  Per image:  {} us", result.per_image_us);
    psp_rt::dprintln!(
        "  Accuracy:   {}/{} ({}%)",
        result.correct,
        result.total,
    ...
    if fd.0 >= 0 {
        ...
        psp_rt::dprintln!("");
        psp_rt::dprintln!("Wrote benchmarks.json to host0:/");
    } else {
        psp_rt::dprintln!("");
        psp_rt::dprintln!("Warning: could not write to host0:/ (psplink not connected?)");
    }
}
