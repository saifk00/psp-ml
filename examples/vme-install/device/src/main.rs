//! One-time installer for the `psp_vme_kernel` VME bootstrap plugin.
//!
...
//! Power-cycle once afterwards and the v1.1 VME job interface works from then on.
//!
...
psp_rt::module!("vme-install", 1, 0);

const SRC: &[u8] = b"host0:/psp_vme_kernel.prx\0";
const DST: &[u8] = b"ms0:/seplugins/psp_vme_kernel.prx\0";
...
const ENTRY: &[u8] = b"ms0:/seplugins/psp_vme_kernel.prx 1\n";
...
fn app_main() {
    ...
    dprintln!("read {} bytes from host0:/psp_vme_kernel.prx", len);

    if !write_plugin(len) {
        return;
    }
    dprintln!("wrote ms0:/seplugins/psp_vme_kernel.prx");
    ...
    dprintln!("");
    dprintln!("DONE. Power-cycle the PSP once, then the VME job interface (v1.1, image mode) is available:");
    dprintln!("  cargo run -p vme-mac-rs-host --release  (or vme-conformance-host)");
}
...
fn register_in_game_txt() -> Option<bool> {
    ...
    if contains(text, b"psp_vme_kernel.prx") {
        return Some(false);
    }
