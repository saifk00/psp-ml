...
//! memory footprint is measurable on its own (same reasoning as the other
//! frontend benchmarks). Both PRXes are staged into one deploy dir.
...
fn main() {
    ...
    for (feature, staged_name) in MODES {
        ...
        let prx = target_dir.join(format!(
            "mipsel-sony-psp/{profile}/birdnet-frontend-benchmark.prx"
        ));
        assert!(
            prx.exists(),
