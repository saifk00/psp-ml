...
(`psp_rt::vme::Job::has_image_mode`; clean sessions restore the marker on
exit). Used by `examples/vme-conformance` to diff real silicon against the
RTL in `vme-emu/` — the harness that calibrated the manual to Issue B.
